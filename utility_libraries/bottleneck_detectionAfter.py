...
def compute_moving_average(deltas, column_names, window_size=3):
    ...
    # Create window_layers to indicate which layers were part of each moving average
    window_layers = []
    for i in range(len(deltas)):
        if i < pad_size:
            # For padded values, use the original single layer
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python bottleneck_after.py <full path csv_file> <full path output_xlsx>")
        sys.exit(1)
    csv_file = sys.argv[1]
    ...
    ##################################################
    bottleneck_list = []
    for class_id, bn in all_bottlenecks.items():
        if bn is not None:
            for layer in bn:
                bottleneck_list.append({'class_id': class_id, 'bottleneck_layer': layer})
    
    bottleneck_report = pd.DataFrame(bottleneck_list)
    bottleneck_csv = os.path.join(result_dir,
